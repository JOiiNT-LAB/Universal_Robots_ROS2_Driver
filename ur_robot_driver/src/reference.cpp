...
OneTaskInvKin::OneTaskInvKin()
{
	...
	q_msr_out.resize(kdl_chain_.getNrOfJoints());
	...
	// sub_des_pos_ = n_.subscribe("rpwc_pose_des", 10, &OneTaskInvKin::callback_des_pose, this);
	sub_des_pos_ = n_.subscribe("command", 10, &OneTaskInvKin::callback_des_pose_stamped, this);
...
void OneTaskInvKin::callback_joint_states(const sensor_msgs::JointState::ConstPtr& msg)
{
	...
	// reset of joint states missing counter 
	count_joint_states_missing = 0 ;

	

	for(int i = 0; i < kdl_chain_.getNrOfJoints(); i++)
	{
		q_msr_out(i) = msg->position[i];
	}

	q_msr_out(0) = msg->position[2]; 
	q_msr_out(1) = msg->position[1]; 
	q_msr_out(2) = msg->position[0]; 
	q_msr_out(3) = msg->position[3]; 
	q_msr_out(4) = msg->position[4]; 
	q_msr_out(5) = msg->position[5]; 
...
void OneTaskInvKin::callback_des_pose_stamped(const geometry_msgs::PoseStamped::ConstPtr& msg)
{
...
void OneTaskInvKin::update()
{
	...
	fk_pos_solver_->JntToCart(q_, x_);	
	fk_pos_solver_->JntToCart(q_msr_out, x_out);
	
	jnt_to_jac_solver_->JntToJac(f, J_);
	...
	Eigen::Vector3d pos, pos_curr_yumi, e_pos, e_quat, pos_out;
	Eigen::Matrix3d orient, orient_curr_yumi, skew, orient_out;
	Eigen::MatrixXd Jac_, Jac_pinv, Jac_pinv_W, tmp_matrix, tmp_matrix_pinv;
	Eigen::Quaterniond quat, quat_curr_yumi, quat_out;
	...
	current_pose_msg.pose.position.x = pos_out(0);
	current_pose_msg.pose.position.y = pos_out(1);
	current_pose_msg.pose.position.z = pos_out(2);
	current_pose_msg.pose.orientation.x = quat_out.x();
	current_pose_msg.pose.orientation.y = quat_out.y();
	current_pose_msg.pose.orientation.z = quat_out.z();
	current_pose_msg.pose.orientation.w = quat_out.w();
...
{	
	...
	switch(step_)
	{
		...
		case 1:
		{
			count_joint_states_missing ++ ;
			// check on the joint states missing
			if (count_joint_states_missing > limit_joint_state_missing){
				ROS_INFO("count_joint_states_missing: %d",count_joint_states_missing) ;
				ROS_ERROR("The system is not receiving the joint states, \n SHUT DOWN INVERSE KINEMATIC CONTROL") ;
				ros::shutdown();
				// da uccidere attualmente e in un futuro forse da sistemare la state machine, tipo passaggio al caso 2 o altro (stato di wainting per i joint states)
			}
